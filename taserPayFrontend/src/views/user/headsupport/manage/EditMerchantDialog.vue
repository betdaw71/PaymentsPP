...
const feeBase = computed (
  () => {
    return {
      ...
      max_limit_in: 0,
      max_limit_out: 0,
      merchant: props.data.merchant_id,
    }
  },
)
...
const updateTrafficTypes = () => {
  baseStore.getTrafficType ({}).then (
    response => {
      if (response.error) {
        throw response.error
      }
      trafficTypes.value = response.data
    },
  ).catch (
    error => {
      snackbar.value = {
...
const removePaymentSystem = index => {
  ...
  merchantStore.deleteMerchantFee ({}, data.value.fees[index].id).then (
    response => {
      if (response.error) {
        throw response.error
      }
...
const savePaymentSystem = index => {
  data.value.fees[index].loading = true
  merchantStore.createMerchantFee ({
    ...
    <VCard class="pa-sm-14 pa-5">
      <VCardItem class="text-center">
        <VCardTitle class="text-h5 mb-3 text-wrap">
          {{ t ('merchant_edit_dialog.title') }} @{{ data.username }}
        </VCardTitle>
        <p class="mb-0">
          {{ t ('merchant_edit_dialog.description') }}
        </p>
      </VCardItem>
                ...
                >
                  ...
                  <VCardText>
                    <VRow>
                      ...
                      >
                        <VTextField
                          v-model="fee.min_limit_in"
                          :label="$t('min_limit_in')"
                          outlined
                          dense
                          :rules="[
                            requiredValidator
                      ...
                      >
                        <VTextField
                          v-model="fee.min_limit_out"
                          :label="$t('min_limit_out')"
                          outlined
                          dense
                          :rules="[
                            requiredValidator
                      ...
                      >
                        <VTextField
                          v-model="fee.max_limit_in"
                          :label="$t('max_limit_in')"
                          outlined
                          dense
                          :rules="[
                            requiredValidator
                      ...
                      >
                        <VTextField
                          v-model="fee.max_limit_out"
                          :label="$t('max_limit_out')"
                          outlined
                          dense
                          :rules="[
                            requiredValidator
