...
const approveWithdrawal = () => {
  tradeStore.approveTradeWithdrawalRequestById ({
    tx_id: withdrawalForm.value.tx_id,
    ...
    response => {
      ...
      snackbar.value = {
        enabled: true,
        type: 'success',
        message: t ('withdrawals.approved'),
      }
    ...
    <VCard class="pa-sm-14 pa-5">
      <VCardItem class="text-center">
        <VCardTitle class="text-h5 mb-3 text-wrap">
          {{ t ('user.withdrawal_approve_dialog.title') }} @{{ withdrawalData.from_user }} (USD {{
            withdrawalData.amount
          }})
        </VCardTitle>
        <p class="mb-0">
          {{ t ('user.withdrawal_approve_dialog.description') }}
        </p>
      </VCardItem>
      ...
      <VCardText>
        ...
        >
          <VRow>
            ...
            >
              <VTextField
                v-model="withdrawalForm.tx_id"
                prepend-inner-icon="tabler-send"
                :label="t ('user.withdrawal_approve_dialog.tx_id')"
                :rules="[
                  requiredValidator,
                ]"
              />
