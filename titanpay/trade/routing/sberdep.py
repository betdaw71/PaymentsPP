...
class SberDepRouting:
    ...
    def get_possible_options(self, risk_cluster, payment_system: PaymentSystem, amount, usd_amount):
        ...
        groups_with_sufficient_balance = PaymentDetailsGroup.objects.filter(trader__balance_usdt__amount__gte=usd_amount, deposit_number_on=True, trader__team__in=teams)
    ...
    def get_details(self, possible_options, active_orders):
        ...
        for group in chosen_group:
            available_details = PaymentDetails.objects.filter(group=group, status=1, deposit_number__isnull=False).exclude(id__in=active_details)
