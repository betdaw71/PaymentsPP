"""Inbound webhooks from Layer-1 PSP (layer-1.io)."""
...
class LayeroneWebhookView(APIView):
    """POST /api/v1/webhooks/psp/layerone/ — callback (state: finished / expired / ...)."""
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not verify_webhook_signature(raw_body, signature):
            logger.warning("LayerOne webhook: invalid signature")
        ...
        if session is None:
            logger.warning("LayerOne webhook: session not found orderId=%s id=%s", order_id, payment_id)
            return Response({"ok": False, "error": "unknown_order"}, status=status.HTTP_404_NOT_FOUND)

        trace_log(
            pay_in=session.pay_in,
            direction=Direction.LAYERONE_WEBHOOK,
            body=body,
            http_method="POST",
    ...
    def _handle_success(self, session: LayeronePayInSession, body: dict) -> Response:
        pay_in = session.pay_in
        ...
        try:
            ...
            logger.warning(
                "LayerOne success webhook: bad InOrder state %s PayIn=%s detail=%s",
                state,
                pay_in.id,
                exc.detail,
            )
    ...
    def _handle_terminal_fail(self, session: LayeronePayInSession) -> Response:
        pay_in = session.pay_in
        if not pay_in or not pay_in.order_id:
            return Response({"ok": True})
        ...
        with transaction.atomic():
            ...
            if locked.status and locked.status.name in ("New", "Money sent by user"):
                try:
                    ...
                    logger.exception("LayerOne webhook deal_time_expired: %s", exc)
            if (
                not inorder_closed
