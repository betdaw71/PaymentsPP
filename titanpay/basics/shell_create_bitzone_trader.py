"""
Django shell: трейдер Bitzone PSP (username bitzone1) + виртуальная группа C2CKZT.

Документация: https://developers.bitzone.space/docs/pay-in/
Webhook: {PUBLIC_API_URL}/api/v1/webhooks/psp/bitzone/ (x-signature = HMAC-SHA256 body, ключ = API key)

После run() в .env:
  BITZONE_API_BASE=https://api.bitzone.space
  BITZONE_API_KEY=...
  BITZONE_TRADER_USERNAME=bitzone1
  BITZONE_PAYIN_METHOD=cross_card   # или card / cross_sbp для KZT — уточнить у Bitzone
...
TEAM_NAME = "Bitzone Agg"
TRADER_USERNAME = "bitzone1"
TRADER_PASSWORD = "ChangeMe_Bitzone_1!"
TRADER_EMAIL = "bitzone1@example.com"
...
def run():
    ...
    user, created = User.objects.get_or_create(
        ...
        defaults={"email": TRADER_EMAIL, "first_name": "Bitzone", "password": TRADER_PASSWORD},
    )
    if created:
    ...
    print("  2) Head support: controlled_teams → «Bitzone Agg»")
    print("  3) MerchantSolution: payment_system=C2CKZT, лимиты под Bitzone")
    print("  4) .env BITZONE_API_KEY + webhook в кабинете Bitzone")
    print(f"  5) Callback: {{PUBLIC_API_URL}}/api/v1/webhooks/psp/bitzone/")
