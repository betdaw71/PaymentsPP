"""
Тест Plutus: отдельная PS C2CKZTTEST → только plutus1 (без prod C2C/C2CKZT).

Создаёт:
  - PaymentSystem C2CKZTTEST (KZT)
  - виртуальную группу + реквизит у plutus1
  - MerchantSolution у тестового мерчанта (по умолчанию melbet_test)

Запуск:
  docker compose exec -T app python manage.py shell < titanpay/basics/shell_setup_plutus_c2ckzttest_routing.py

Проверка роутинга:
  docker compose exec app python manage.py diagnose_routing --merchant melbet_test --ps C2CKZTTEST --amount 5000

Pay-in (H2H): payment_system=C2CKZTTEST, currency=KZT, ftd=false

Успех от Plutus:
  - попросить Plutus закрыть сделку completed (platform_id = UUID PayIn)
  - или sandbox: PLUTUS_API_KEY=th_sandbox_... и сумма 100 → колбек через ~5 с

Локально прогнать колбек (без Plutus), после реальной выдачи реквизитов:
  docker compose exec app python manage.py shell -c "
from basics.shell_setup_plutus_c2ckzttest_routing import simulate_plutus_success_webhook
simulate_plutus_success_webhook('PAY_IN_UUID')
"
"""
...
GROUP_OWNER = "Plutus C2CKZTTEST test"
LIMITS = {
...
def ensure_plutus_virtual_group(trader: Trader, ps: PaymentSystem, kzt: Currency, traffic: TrafficType):
...
def run(merchant_username: str = MERCHANT_USERNAME) -> None:
    print("=" * 60)
    print(f"Plutus test routing: PS={PS_NAME} merchant={merchant_username}")
    print("=" * 60)
    ...
    if trader_user is None:
        raise RuntimeError(
            f"Trader user {plutus_trader_username()!r} not found — run shell_create_plutus_trader.py first"
        )
    trader = Trader.objects.filter(user=trader_user).select_related("team").first()
    if trader is None:
        raise RuntimeError(f"Trader {plutus_trader_username()!r} not found")
    ...
    print("")
    print("Done.")
    print(f"  • .env: PLUTUS_PAYMETHOD_MAP должен содержать \"{PS_NAME}\":\"c2c\" (или дефолт в коде)")
    print(f"  • Pay-in: payment_system={PS_NAME}, currency=KZT")
    print("  • Webhook Plutus: platform_id = <PayIn UUID>")
    print(f"  • diagnose: python manage.py diagnose_routing --merchant {merchant_username} --ps {PS_NAME} --amount 5000")


def simulate_plutus_success_webhook(pay_in_id: str, *, received_amount: str | None = None) -> None:
    """
    Имитация колбека Plutus status=completed (для проверки complete без ожидания провайдера).
    ...
    amount = received_amount or str(pay_in.amount)
    body = {
        "platform_id": str(pay_in.id),
        "status": "completed",
        "amount": amount,
