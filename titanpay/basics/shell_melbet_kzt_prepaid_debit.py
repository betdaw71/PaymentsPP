...
Этот прогон: 2500 USDT × 479.3 = 1 198 250.00 KZT.

Сначала осмотр:
  docker compose exec -T -e DRY_RUN=1 app python manage.py shell \
    < titanpay/basics/shell_melbet_kzt_prepaid_debit.py

Списание:
  docker compose exec -T -e DRY_RUN=0 app python manage.py shell \
    < titanpay/basics/shell_melbet_kzt_prepaid_debit.py

Из контейнера (/app):
  DRY_RUN=1 python manage.py shell < basics/shell_melbet_kzt_prepaid_debit.py
  DRY_RUN=0 python manage.py shell < basics/shell_melbet_kzt_prepaid_debit.py

Переопределение: USDT, RATE, MERCHANT_USERNAME, COMMENT.
"""
...
USDT = Decimal(os.environ.get("USDT", "2500"))
RATE = Decimal(os.environ.get("RATE", "479.3"))
...
def run() -> None:
    ...
    if KZT != Decimal("1198250.00"):
        print(f"  ! warning: kzt is {KZT}, expected 1198250.00 for 2500 @ 479.3")
