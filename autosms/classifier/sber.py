...
class Sber:
    ...
    def regex_list(self):
        return [
            ...
            {"regex": "СЧЁТ(\d{4}) .*? (?:Оплата|перевод) (\d{1,4}(?: \d{3})*)р.*? Баланс: (\d+(?: \d{3})*(?:\.\d{2})?)р", "fields": ["deposit_number", "amount", "balance"], "direction": "out", "methods": ["Sber", "SBP"]},
            {"regex": "(?:MIR-|VISA|МИР-|ECMC|УЭК|МИР|MIR)(\d{4}) .*? зачисление (\d{1,4}(?: \d{3})*)р .*? Баланс: (\d+(?: \d{3})*(?:\.\d{2})?)р", "fields": ["card_number", "amount", "balance"], "direction": "in", "methods": ["Sber", "SberDep"]},
            ...
            {"regex": "СЧЁТ(\d{4}) .*? Перевод (\d{1,4}(?: \d{3})*)р от .*? Баланс: (\d+(?: \d{3})*(?:\.\d{2})?)р", "fields": ["deposit_number", "amount", "balance"], "direction": "in", "methods": ["SBP", "SberPay", "SberDep"]},
            {"regex": "СЧЁТ(\d{4}) .*? Зачисление (\d{1,4}(?: \d{3})*)р Баланс: (\d+(?: \d{3})*(?:\.\d{2})?)р", "fields": ["deposit_number", "amount", "balance"], "direction": "in", "methods": ["SberDep"]},
            ...
            {"regex": "СЧЁТ(\d{4}) .*? зачисление (\d{1,4}(?: \d{3})*)р со вклада Баланс: (\d+(?: \d{3})*(?:\.\d{2})?)р", "fields": ["deposit_number", "amount", "balance"], "direction": "in", "methods": ["SberDep"]},
        ]
